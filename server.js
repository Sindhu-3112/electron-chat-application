...
io.on('connection', (socket) => {
  ...
  socket.on('sendGroupMessage', (data) => {
    ...
    console.log(
      `[NOTIFICATION DEBUG] Group notification sent to room ${roomId} from ${message.user}`
    )
  })
    ...
    if (room && room.creator === socket.id) {
      ...
      if (action === 'remove') {
        ...
        const updatedDetails = room.members.map((id) => ({
          id,
          name: connectedUsers[id] || 'Unknown'
        }))
  ...
  socket.on('delete_message', ({ messageId, recipientId, type }) => {
    const senderUsername = socket.username || 'TheSender'

    if (type === 'everyone') {
      io.to(recipientId).emit('message_deleted', {
        messageId,
        chatPartnerId: senderUsername
      })
    }
